A ==**random measure**== is a locally finite [[Transition Kernels|transition kernel]] $\zeta$ from a probability space $(\Omega, \FFF,\PP)$ to a separable complete metric space $(E, \GGG)$. Local finiteness means that $\zeta(\omega, B) < \infty$ for all bounded measurable sets $B\in \GGG$, almost surely.
