...
> [!definition] Pre-Hilbert Space
 ...
 \notation{From now on, $V$ always denotes a pre-Hilbert space.} From basic linear algebra, we know 
> [!theorem] Cauchy-Schwarz Inequality
 Let $V$ be a pre-Hilbert space. Then for all $v,w\in V$, $$ \abs{\braket{v|w}} \leq \norm{v}\norm{w} $$ 
...
> [!theorem] Parallelogram Law
 Let $V$ be a pre-Hilbert space. Then for all $v,w\in V$, $$ \norm{v+w}^2 + \norm{v-w}^2 = 2\norm{v}^2 + 2\norm{w}^2 $$ 
...
> [!definition] Orthogonal
 ...
 We are go for fucking launch. Let the topology fire. Our first main theorem, a stepping stone for all future work: 
...
> [!solution] 
 ...
 Well, the theorem follows by definition. In particular, we are guaranteed to have only countably many nonzero terms. 
> [!definition] Maximal
...
> [!problem] Separable Spaces
 \label{prob:separable} Every ==**separable**== pre-Hilbert space has a countable maximal orthonormal subset. 
...
> [!definition] Hilbert Space
 ...
 \notation{From now on, $\HH$ denotes a Hilbert Space.} We literally have exactly one example of this: 
> [!example] $L^2(E)$
...
> [!theorem] Fourier-Bessel Series
 Let $S$ be a maximal orthonormal basis for $\HH$. Then, for all $v\in \HH$, $$ v = \sum_{e\in S} \braket{e| v} e $$ where the sum converges in norm. 
...
> [!proof] 
 ...
 Thus, we have a partial converse to \href{prob:separable}{Problem \ref{prob:separable}}, which also shows why we like the separability condition: 
...
> [!theorem] Everything is $\ell^2$
 Let $\HH$ be a separable Hilbert space. Then there exists a bijective bounded linear operator $T: \HH\to \ell^2$ such that for all $u,v\in \HH$, $$ \braket{u| v} = \braket{Tu | Tv} $$ In other words, $\HH$ is ==**isometrically isomorphic**== to $\ell^2$. 
