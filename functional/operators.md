...
>  
 ...
 Now, the main thing to update your intuition on is that **we only care about bounded linear maps.** This causes a lot of vanilla linear algebra to fail. 
> [!example] Bijectivity, Injectivity, Surjectivity
 In the space $\ell^2$ of square summable sequences, the operator $$ Ta = \left\{ \frac{a_1}{1}, \frac{a_2}{2}, \frac{a_3}{3}, \dots \right\} $$ is continuous and injective, but not surjective. For instance, it cannot hit the sequence $b = (1, 1/2, 1/3, \dots)\in \ell^2$. Thus $T$ is not invertible. If it were, then the inverse would be $$ T^{-1}a = \left\{ a_1, 2a_2, 3a_3, \dots \right\} $$ which is not bounded. This will cause some subtleties when we talk about eigenanalysis later. 
