...
$\so(V)\cong \wedge^2 V$ as vector spaces via the "plane of rotation" embedding. We want to extend this to an algebra map from the universal envelope of $\so(V)$ to $\wedge^2V\subset \wedge V$. The induced algebra is called $\Cl(V)$.
...
>$V$ is endowed with a metric $\eta$. $\so(V)$ consists of elements $\omega^\alpha_\beta$ such that $\omega^{\alpha\beta} + \omega^{\beta\alpha} = 0$. It's literally completely obvious what's happening now; the map $\so(V)\to \wedge^2V$ is literally contraction against $eta$.
...
Thus we have an embedding $\xi:\so(V)\to \Cl(V)$. Here's a [[story about Clifford Algebras]].
...
# $\Cl(V)$ Characterization

In the same way that the [[Universal Enveloping Algebra]] is a [[deformation]] of $S(V)$ (roughly speaking, a valid first-order perturbation to the algebra operator $V^*\otimes V^*\otimes V$), the Clifford algebra is a deformation of $\wedge^n V$. Thus, we have an associated Poincare-Birkhoff-Witt-like theorem.

> [!idea]- Deformation?
> See [[filtered algebra]]. The natural filtration $V\in F_1$ induces a surjective map $\phi: \bigwedge V\to \gr\Cl(V)$. This deformation will end up being "flat" (the semidirected nature breaks in an good way) and $\phi$ is an isomorphism.
...
> [!theorem] When $\dim V = 2n+1$, $\Cl(V)\cong \Mat_{2^n}(k)$
...
Then, $\xi^*M$ is a representation of $\so_{2n}$ on a space $\cong \CC^{2^n}$ via $\rho_{\xi^*M}(a) = \rho_M(\xi(a))$. This representation actually reduces into $(\xi^*M)_0\oplus (\xi^*M)_1$ consisting of the even and odd-degree parts, which are isomorphic to $S_+$ and $S_-$.  $\xi^*M_\pm$ are representations of $\so_{2n+1}$ on spaces $\cong \CC^{2^n}$, and they turn out to both be isomorphic to $S$.
