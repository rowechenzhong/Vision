...
> [!example] $\SL_3\curvearrowright \wedge^2\CC^3$
> The Lie group $\SL_3$ operates via the exponential map a la [[Fundamental Theorems of Lie Theory]]. In our specific framework, its good to note that diagonal matrices $e^h$ in $\SL_3$ exponentiate from $h\in \hh$, and so act by a scalar. Specifically, on weight space $\lambda \in \hh^*$, $h$ acts via $\lambda(h)$, thus $e^h$ acts via $e^{\lambda(h)}$.
